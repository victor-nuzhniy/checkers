...
function connectStartSocket() {
  ...
  startSocket.onclose = function (e) {
    console.log(
      'WebSocket startSocket connection closed unexpectedly. Trying to reconnect in 2s...'
    );
    setTimeout(function () {
...
startSocket.onmessage = function (e) {
  ...
  if (data.type === 'start_playing') {
    const playerStatus = document.getElementById(data.message.player_pk);
    playerStatus.innerHTML = 'Busy';
    playerStatus.removeAttribute('style');
  } else if (data.type === 'game_over') {
    const playerStatus = document.getElementById(data.message.user_id);
    const rivalStatus = document.getElementById(data.message.rival_id);
    ...
    const addedUserId = data.message.user_id;
    ...
    if (data.message.rival_id === currentUserId) {
      ...
      const link =
        'http://' + window.location.host + '/' + data.message.player_id + '/' + currentUserId + '/';
    ...
    if (data.message.rival_id === currentUserId) {
      ...
      const a = document.createElement('a');
      const link =
        'http://' + window.location.host + '/' + currentUserId + '/' + data.message.player_id + '/';
      a.setAttribute('href', link);
...
const statuses = document.getElementsByClassName('ready-to-play');
